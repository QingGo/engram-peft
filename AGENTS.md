...
- **覆盖率要求**：核心模块（`src/engram_peft/model.py`）的单元测试覆盖率必须 ≥ 90%。
...
- 模板依赖：不存在则自动基于 `.agent_memory.template.md` 创建

### 10.1 读机制
每次开启新复杂任务/跨文件重构前，**必须先读取 `memory-update` 技能规范 + 主动读取 `.agent_memory.md`** 恢复上下文，不依赖全局索引。

### 10.2 强制写入场景（短期+长期）
以下情况**必须执行记忆更新**，且严格遵循 `memory-update` 技能规范：
1. 短期：当前会话产生新临时TODO、即时踩坑、未解决问题
2. 短期：会话即将结束，需归档当前上下文
3. 长期：核心架构决策、人类否决/确认方案
4. 长期：死循环熔断后，记录排错过程与猜想
5. 长期：修复顽固性Bug，沉淀通用踩坑点

### 10.3 标准记忆Schema
更新时**禁止自由格式**，必须严格使用以下结构：
```markdown
## 🎯 Current Milestone
- [ ] 当前高优目标与进度
...
### 10.4 记忆修剪规则
- 短期记忆：会话结束后**自动清理已完成临时TODO、过期上下文**
